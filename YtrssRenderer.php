...
/**
 * YtRssRenderer is a feed that implements RSS 2.0 Specification
 *
 ...
 */
class YtRssRenderer extends DocumentRenderer
{
	/**
	 * Renderer mime type
	 *
	...
	public function render($name = '', $params = null, $content = null)
	{
		...
		$feed = "<rss version=\"2.0\" xmlns:yandex=\"http://news.yandex.ru\" xmlns:media=\"http://search.yahoo.com/mrss/\" xmlns:turbo=\"http://turbo.yandex.ru\">\n";
		$feed .= "	<channel>\n";
		$feed .= "		<title>" . $feed_title . "</title>\n";
		$feed .= "		<description>" . $data->getDescription() . "</description>\n";
		$feed .= "		<link>" . str_replace(' ', '%20', $url) . "</link>\n";
		...
		{
			//var_dump($data->items[$i]);
			$itemlink = $data->items[$i]->link;
			
			$header = "<header><h1>" . $data->items[$i]->title . "</h1></header>";
			...
			$feed .= "			<turbo:content><![CDATA[" . $header . $this->_convertLinks($data->items[$i]->description, $itemlink) . "]]></turbo:content>\n";

			if ($data->items[$i]->authorEmail != '')
			{
				$feed .= "			<author>"
					. htmlspecialchars($data->items[$i]->authorEmail, ENT_COMPAT, 'UTF-8') . "</author>\n";
			}
	...
	protected function _convertLinks($text, $articleURL)
	{
		$base = \JUri::base();
		$text = preg_replace("/(href|src)=\"(?!http|ftp|https|mailto|data|#)([^\"]*)\"/", "$1=\"$base\$2\"", $text);
		$text = preg_replace("/(href)=\"#([^\"]*)\"/", "$1=\"$articleURL#\$2\"", $text);
